...
export const useDialog = (type: DialogType = DIALOG_TYPE.LISTBOX) => {
    ...
    (e: React.MouseEvent) => {
      ...
      if (type === DIALOG_TYPE.LISTBOX) {
        ...
        setTimeout(() => {
          ...
          const focusableNodeList = dialogRef.current.querySelectorAll(
  ...
  const handleListboxKeyTrap = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (dialogRef.current) {
        const optionNodeList = Array.prototype.slice.call(
          dialogRef.current.querySelectorAll("[role='option']"),
        );
        if (!optionNodeList.length) return;
  ...
  const getParseMenuKeyTrap = (e: KeyboardEvent) => {
    const menuNodeList = Array.prototype.slice.call(
      dialogRef.current!.querySelectorAll("[role='menuitem']"),
    );
    const eventTarget = e.target;
  ...
  useEffect(() => {
    ...
    const handleKeyListener = (e: KeyboardEvent) => {
      const listener =
        type === DIALOG_TYPE.LISTBOX
          ? listboxKeyListenerMap.get(e.key)
          : menukeyListenerMap.get(e.key);
      if (listener) {
        listener(e);
      }
    };
